...
icon_font = ImageFont.truetype('lineawesome-webfont.ttf',ICONSIZE)
...
#    cmd = "top -bn1 | grep load | awk '{printf \"CPU: %.0f%%\", $(NF-2) * 25}'"
    cmd = "top -bn1 | grep load | awk '{cpu=$(NF-2)*25; if (cpu < 1) cpu=1; printf \"%.0f%%\", cpu}'"
...
#    cmd = "free -m | awk 'NR==2{printf \"RAM: %s/%sM %d%%\", $3,$2,($3*100/$2)+0.5 }'"
    cmd = "free -m | awk 'NR==2{printf \"%.2f/%.1fG %d%%\", $3/1024,$2/1024,($3*100/$2)+0.5 }'"
